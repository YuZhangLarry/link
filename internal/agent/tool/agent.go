...
// NewAgent 创建 Agent
func NewAgent(chatModel model.BaseChatModel, config *AgentConfig) (*Agent, error) {
	if config == nil {
	...
	if len(config.ToolNames) > 0 {
		registry, err = InitCustomTools(config.ToolNames)
		if err != nil {
...
func NewAgentWithRegistry(chatModel model.BaseChatModel, config *AgentConfig, registry *Registry) (*Agent, error) {
	if config == nil {
		config = DefaultAgentConfig()
	}
...
func (a *Agent) executeToolCalls(ctx context.Context, toolCalls []*schema.ToolCall) []*ToolExecResult {
	// 使用超时上下文
...
func (a *Agent) GetToolRegistry() *Registry {
	return a.registry
}
