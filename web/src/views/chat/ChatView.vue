  ...
  <div class="chat-page">
    ...
    <main class="main-content">
      ...
      <div class="messages-area" ref="messagesContainer">
        <!-- 普通模式的空状态 -->
        <div v-if="chatMode === 'normal' && messages.length === 0 && !streamingContent" class="empty-state">
        ...
        <!-- Agent 模式的空状态 -->
        <div v-if="chatMode === 'agent' && agentMessages.length === 0 && !isAgentStreaming" class="empty-state">
          <div class="empty-icon agent-icon">
            <svg width="64" height="64" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5">
              <circle cx="12" cy="12" r="10"/>
              <path d="M12 16v-4M12 8h.01"/>
            </svg>
          ...
          <p>Agent 智能助手</p>
          <p class="hint">我可以使用多种工具来帮助你获取信息</p>
        </div>

        <!-- 普通模式消息列表 -->
        <div v-if="chatMode === 'normal'" class="messages-list">
          <!-- RAG 上下文提示 -->
          <div v-if="currentRAGContext" class="rag-context-banner">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
        ...
        <!-- Agent 模式消息列表 -->
        <div v-if="chatMode === 'agent'" class="messages-list agent-mode">
          <div
          ...
          >
            ...
            <div v-else class="agent-response">
              ...
              <div class="thinking-steps">
                ...
                >
                  <!-- 工具调用步骤 -->
                  <template v-if="step.type === 'action' || step.type === 'utility'">
                    <div class="step-header">
                      <span class="step-number">步骤 {{ step.step }}</span>
                      <span class="step-type">{{ step.tool_desc || '调用工具' }}</span>
                      <span v-if="step.stage" class="step-stage">{{ step.stage }}</span>
                    ...
                    <div class="step-content">
                      ...
                      <div class="tool-call-card">
                        ...
                        <div v-if="step.tool_params && Object.keys(step.tool_params).length > 0" class="tool-params">
                        ...
                        <div v-if="step.tool_output" class="tool-output">
                          ...
                          <span class="output-value">{{ truncateText(step.tool_output, 200) }}</span>
                        </div>
                      </div>
                    </div>
                  </template>
                  ...
                  <!-- 子 Agent 调用步骤 -->
                  <template v-else-if="step.type === 'agent_call'">
                    <div class="step-header">
                      <span class="step-number">步骤 {{ step.step }}</span>
                      <span class="step-type agent-call">{{ step.agent_name || step.tool_desc || '子代理调用' }}</span>
                      <span v-if="step.agent_stage" class="step-stage">{{ step.agent_stage }}</span>
                      <span class="step-time">{{ formatStepTime(step.timestamp) }}</span>
                    </div>
                    <div class="step-content">
                      <div class="agent-call-card">
                        <div class="agent-info">
                          <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                            <path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-6 0V5a3 3 0 0 0-3 3"/>
                            <path d="M19 9a3 3 0 0 0-3-3H8a3 3 0 0 0-3 3v8a3 3 0 0 0 3 3h8a3 3 0 0 0 3-3V9z"/>
                            <path d="M12 12a2 2 0 1 0 0-4 2 2 0 0 0 0 4"/>
                            <path d="M12 18v-6"/>
                          </svg>
                          <div class="agent-details">
                            <div class="agent-name">{{ step.agent_name || step.tool_name }}</div>
                            <div v-if="step.tool_params && Object.keys(step.tool_params).length > 0" class="tool-params">
                  ...
                  <!-- 检索步骤 -->
                  <template v-else-if="step.type === 'search'">
                    <div class="step-header">
                      <span class="step-number">步骤 {{ step.step }}</span>
                      <span class="step-type search">{{ step.tool_desc || '检索' }}</span>
                      <span v-if="step.stage" class="step-stage">{{ step.stage }}</span>
                      <span class="step-time">{{ formatStepTime(step.timestamp) }}</span>
                    </div>
                    <div class="step-content">
                      <div class="search-card">
                        <div class="search-info">
                          ...
                          <span class="search-name">{{ step.tool_name }}</span>
                        </div>
                        <div v-if="step.tool_params && Object.keys(step.tool_params).length > 0" class="tool-params">
                  ...
                  <!-- 思考步骤 -->
                  <template v-else-if="step.type === 'thought' || step.type === 'thinking'">
                    <div class="step-header">
                      <span class="step-number">步骤 {{ step.step }}</span>
                  ...
                  <template v-else-if="step.type === 'plan'">
                    <div class="step-header">
                      <span class="step-number">步骤 {{ step.step }}</span>
                      <span class="step-type plan">📋 规划</span>
                      <span class="step-time">{{ formatStepTime(step.timestamp) }}</span>
                    </div>
                    <div class="step-content">
                      <div class="plan-content" v-html="renderMarkdown(step.content || '')"></div>
                    </div>
                  </template>
                  ...
                  <template v-else-if="step.type === 'analysis'">
                    <div class="step-header">
                      <span class="step-number">步骤 {{ step.step }}</span>
                      <span class="step-type analysis">🔍 分析</span>
                      <span class="step-time">{{ formatStepTime(step.timestamp) }}</span>
                    </div>
                    <div class="step-content">
                      <div class="analysis-content" v-html="renderMarkdown(step.content || '')"></div>
                    </div>
                  </template>

                  <!-- 评审步骤 -->
                  <template v-else-if="step.type === 'review'">
                    <div class="step-header">
                      <span class="step-number">步骤 {{ step.step }}</span>
                      <span class="step-type review">✓ 评审</span>
                      <span class="step-time">{{ formatStepTime(step.timestamp) }}</span>
                    </div>
                    <div class="step-content">
                      <div class="review-content" v-html="renderMarkdown(step.content || '')"></div>
                    </div>
                  </template>

                  <!-- 合成步骤 -->
                  <template v-else-if="step.type === 'synthesis'">
                    <div class="step-header">
                      <span class="step-number">步骤 {{ step.step }}</span>
                      <span class="step-type synthesis">📝 合成</span>
                      <span class="step-time">{{ formatStepTime(step.timestamp) }}</span>
                    </div>
                    <div class="step-content">
                      <div class="synthesis-content" v-html="renderMarkdown(step.content || '')"></div>
                    </div>
                  </template>

                  <!-- 检索结果步骤 -->
                  <template v-else-if="step.type === 'retrieval' || step.type === 'tool_result'">
                    <div class="step-header">
                      <span class="step-number">步骤 {{ step.step }}</span>
                      <span class="step-type retrieval">📊 结果</span>
                      <span v-if="step.related_tool" class="related-tool">来自: {{ step.related_tool }}</span>
                      <span class="step-time">{{ formatStepTime(step.timestamp) }}</span>
                    </div>
                    <div class="step-content">
                      <div class="retrieval-content" v-html="renderMarkdown(step.content || '')"></div>
                    </div>
                  </template>
              ...
              <!-- 最终答案 -->
              <div v-if="msg.answer" class="agent-answer">
                <div class="answer-label">最终答案</div>
                <div class="answer-content" v-html="renderMarkdown(msg.answer)"></div>
              </div>
            </div>
          </div>

          <!-- Agent 流式输出 -->
          <div v-if="isAgentStreaming" class="agent-message streaming">
            <div class="agent-response">
              ...
              <div class="thinking-steps">
                ...
                >
                  <!-- 子 Agent 调用步骤 -->
                  <template v-if="step.type === 'agent_call'">
                    <div class="step-header">
                      <span class="step-number">步骤 {{ step.step }}</span>
                      <span class="step-type agent-call">{{ step.agent_name || step.tool_desc || '子代理调用' }}</span>
                      <span v-if="step.agent_stage" class="step-stage">{{ step.agent_stage }}</span>
                  ...
                  <!-- 工具调用步骤 -->
                  <template v-else-if="step.type === 'action' || step.type === 'utility'">
                    <div class="step-header">
                      <span class="step-number">步骤 {{ step.step }}</span>
                      <span class="step-type">{{ step.tool_desc || '工具调用' }}</span>
                      <span v-if="step.stage" class="step-stage">{{ step.stage }}</span>
                      <span v-if="step.isActive" class="step-status">执行中...</span>
                    </div>
                    <div class="step-content">
                  ...
                  <template v-else>
                    ...
                    <div class="step-content">
                      <div class="default-content" v-html="renderMarkdown(step.content || '')"></div>
                    </div>
                  </template>
                </div>
              </div>
...
// 聊天模式: 'normal' | 'agent'
const chatMode = ref<'normal' | 'agent'>('normal')
...
// 切换聊天模式
function switchMode(mode: 'normal' | 'agent') {
  chatMode.value = mode
...
function parseAgentSteps(agentStepsJson: string | undefined | null): AgentStep[] {
  ...
  try {
    ...
    console.error('Failed to parse agent_steps:', e, 'Raw data:', agentStepsJson)
    return []
  }
}
  ...
  try {
    ...
    for await (const event of streamAgentChat({
      ...
      if (event.event === 'step') {
        ...
        // 更新当前步骤的状态
        if (currentAgentSteps.value.length > 0) {
    ...
    // 注意：不重建 agentMessages，因为我们已经在前端构建了带有步骤的消息
    if (currentSessionId.value) {
      const res = await sessionApi.getDetail(currentSessionId.value)
      if (res.data) {
        messages.value = res.data.messages || []
        // 不调用 buildAgentMessages，保留前端构建的 agentMessages（包含步骤）
      }
    }
  } catch (error) {
...
// 格式化工具参数
function formatToolParams(params: Record<string, any>): string {
  if (!params) return ''
  try {
...
.message.streaming .message-text {
  border-left: 3px solid rgba(255, 126, 179, 0.8);
}
...
.agent-avatar {
  ...
  height: 36px;
  border-radius: 50%;
  background: rgba(122, 252, 255, 0.6);
  display: flex;
  align-items: center;
  justify-content: center;
  color: white;
...
@keyframes glow {
  0%, 100% {
    box-shadow: 0 0 5px rgba(122, 252, 255, 0.5);
  }
  50% {
...
.step-item.complete {
  border: 1px solid #7affc3;
  background: rgba(122, 255, 195, 0.1);
}
...
@keyframes borderPulse {
  0%, 100% { border-color: rgba(255, 126, 179, 0.8); }
  50% { border-color: rgba(255, 126, 179, 0.4); }
}
...
.checkbox-item input[type="checkbox"] {
  ...
  height: 18px;
  cursor: pointer;
  accent-color: rgba(255, 126, 179, 0.8);
}
